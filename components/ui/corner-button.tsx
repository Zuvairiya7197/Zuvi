...
export interface CornerButtonProps
  ...
  showIcon?: boolean;
  /** Accent colour used for the button background and glow. @default "#d6b36a" */
  accentColor?: string;
  /** Extra classes applied to the outer wrapper div. */
  wrapperClassName?: string;
  /** Element/component to render the trigger as (e.g. Next.js `Link`). @default "button" */
  as?: React.ElementType;
  /** Additional props forwarded to the rendered element (e.g. `href` when `as={Link}`). */
  [key: string]: unknown;
}
  ...
  return (
    ...
    >
      ...
      <style>{`
        ...
        /* ── Touch devices: no hover, so loop the reveal automatically ── */
        @media (hover: none) {
          .corner-btn-wrapper {
            animation: corner-bg-change-loop calc(var(--speed) * 8) ease-in-out infinite;
          }
          @keyframes corner-bg-change-loop {
            0%, 40%  { background-color: transparent; }
            60%, 100% { background-color: var(--accent-glow); }
          }

          .corner-btn-wrapper .corner-dot.top.left {
            animation: corner-dot-tl-loop calc(var(--speed) * 8) ease-in-out infinite;
          }
          @keyframes corner-dot-tl-loop {
            0%, 40%  { top: 50%; left: 20%; opacity: 0; }
            55% { opacity: 0.6; }
            60%, 100% { top: calc(var(--dot-size) * -0.5); left: calc(var(--dot-size) * -0.5); opacity: 1; }
          }
          .corner-btn-wrapper .corner-dot.top.right {
            animation: corner-dot-tr-loop calc(var(--speed) * 8) ease-in-out infinite;
            animation-delay: calc(var(--speed) * 0.6);
          }
          @keyframes corner-dot-tr-loop {
            0%, 40%  { top: 50%; right: 20%; opacity: 0; }
            55% { opacity: 0.6; }
            60%, 100% { top: calc(var(--dot-size) * -0.5); right: calc(var(--dot-size) * -0.5); opacity: 1; }
          }
          .corner-btn-wrapper .corner-dot.bottom.right {
            animation: corner-dot-br-loop calc(var(--speed) * 8) ease-in-out infinite;
            animation-delay: calc(var(--speed) * 1.2);
          }
          @keyframes corner-dot-br-loop {
            0%, 40%  { bottom: 50%; right: 20%; opacity: 0; }
            55% { opacity: 0.6; }
            60%, 100% { bottom: calc(var(--dot-size) * -0.5); right: calc(var(--dot-size) * -0.5); opacity: 1; }
          }
          .corner-btn-wrapper .corner-dot.bottom.left {
            animation: corner-dot-bl-loop calc(var(--speed) * 8) ease-in-out infinite;
            animation-delay: calc(var(--speed) * 1.8);
          }
          @keyframes corner-dot-bl-loop {
            0%, 40%  { bottom: 50%; left: 20%; opacity: 0; }
            55% { opacity: 0.6; }
            60%, 100% { bottom: calc(var(--dot-size) * -0.5); left: calc(var(--dot-size) * -0.5); opacity: 1; }
          }

          .corner-btn-wrapper .corner-line.top {
            animation: corner-line-top-loop calc(var(--speed) * 8) ease-in-out infinite;
            animation-delay: calc(var(--speed) * 0.8);
          }
          @keyframes corner-line-top-loop {
            0%, 40%  { transform: rotate(5deg) scaleX(0); }
            60%, 100% { transform: rotate(0deg) scaleX(1); }
          }
          .corner-btn-wrapper .corner-line.bottom {
            animation: corner-line-bottom-loop calc(var(--speed) * 8) ease-in-out infinite;
            animation-delay: calc(var(--speed) * 2);
          }
          @keyframes corner-line-bottom-loop {
            0%, 40%  { transform: rotate(5deg) scaleX(0); }
            60%, 100% { transform: rotate(0deg) scaleX(1); }
          }
          .corner-btn-wrapper .corner-line.left {
            animation: corner-line-left-loop calc(var(--speed) * 8) ease-in-out infinite;
            animation-delay: calc(var(--speed) * 2.4);
          }
          @keyframes corner-line-left-loop {
            0%, 40%  { transform: scaleY(0); }
            60%, 100% { transform: scaleY(1); }
          }
          .corner-btn-wrapper .corner-line.right {
            animation: corner-line-right-loop calc(var(--speed) * 8) ease-in-out infinite;
            animation-delay: calc(var(--speed) * 1.4);
          }
          @keyframes corner-line-right-loop {
            0%, 40%  { transform: rotate(5deg) scaleY(0); }
            60%, 100% { transform: rotate(0deg) scaleY(1); }
          }

          .corner-btn {
            animation: corner-btn-pulse-loop calc(var(--speed) * 8) ease-in-out infinite;
          }
          @keyframes corner-btn-pulse-loop {
            0%, 40%  { background-color: var(--accent); border-radius: 999px; }
            60%, 100% { background-color: #fff; border-radius: 10% / 200%; }
          }

          .corner-btn-svg {
            animation: corner-btn-svg-loop calc(var(--speed) * 8) ease-in-out infinite;
          }
          @keyframes corner-btn-svg-loop {
            0%, 40%  { stroke: #0007; fill: #fffa; }
            60%, 100% { stroke: #0008; fill: var(--accent); }
          }
        }
      `}</style>
    </div>
